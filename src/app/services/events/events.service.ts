	...
	getEventbriteEvents(wordCount: number): Promise<Event[]> {
		return this.http
			...
			.then((response: { events: any[] }) =>
				response.events.map(
					event =>
						event && // This is to solve test event issue.
						event.logo && {
							title: event.name.text,
							description: event.description.text,
							summary:
								event.description.text.length < wordCount
									? event.description.text
									: event.description.text.substr(
											0,
											event.description.text.lastIndexOf(' ', wordCount)
									  ) + '...',
