...
function TyperSetup() {
    ...
    var elements2 = document.getElementsByClassName("typer-stop");
    for (const e of elements2) {
        let owner = typers[e.dataset.owner];
        e.onclick = function() { owner.stop(); };
    }

    var elements3 = document.getElementsByClassName("typer-start");
    for (const e of elements3) {
        let owner = typers[e.dataset.owner];
        e.onclick = function() { owner.start(); };
    }

    var elements4 = document.getElementsByClassName("cursor");
    for (const e of elements4) {
        let t = new Cursor(e);
